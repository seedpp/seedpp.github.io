...
2. Upload `index.html` and the `image/` folder to the repository root, keeping the folder
   structure above. `image.zip` is the same folder zipped, for convenience when transferring. Add this `README.md` too if you want.
...
Point that person's `<img class="avatar avatar--photo">` at a new file — `src="image/name.jpg"`
works, as long as the `image/` folder ships with the page. A square image around 420×420 is
